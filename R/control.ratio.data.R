...
control.ratio.data <- function(data, positive.control)
{
  ...
  data.join <- full_join(data_0, data, by =c('which_row', 'column', 'sample', 'bacteria', 'experiment_id'))
  ...
  data.join <- data.join[,-which(names(data.join) %in% c('measure.y', 'measure.x'))]
  ...
  data.join <- full_join(data_pos,data.join,by = c( "bacteria", "experiment_id", "time.y"))
  
  
  data.join$deltaratio <- 1- (data.join$mean.y  / data.join$mean.x)
